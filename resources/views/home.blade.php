                ...
                @forelse ($artikel as $key => $value)
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            <img src="{{ asset('uploads/' . $value->gambar) }}" class="card-img-top" alt="Gambar Artikel">
                            <div class="card-body">
                                <h5 class="card-title">{{ $value->judul }}</h5>
                                <p class="card-text">{{ Str::limit($value->konten, 100) }}</p>
                                <a href="{{ url('artikeldetail', $value->idartikel) }}" class="btn btn-primary">Baca
                                    Selengkapnya</a>
                            </div>
                        </div>
                    </div>
                ...
                @forelse ($produk as $item)
                    <div class="col-md-4 mb-4">
                        <div class="card h-100 shadow-sm">
                            <a href="{{ url('produkdetail/' . $item->idproduk) }}"
                                style="text-decoration: none; color: inherit;">
                                <img src="{{ asset('uploads/' . $item->gambar) }}" class="card-img-top" alt="Produk">
                                <div class="card-body d-flex flex-column justify-content-between">
                                    <h6 class="card-title fw-bold text-uppercase">{{ $item->nama_produk }}</h6>
                                    <p class="card-text text-muted small">
                                        {{ \Illuminate\Support\Str::limit($item->deskripsi, 35) }}</p>
                                </div>
                            </a>
                            ...
                            <div class="card-footer bg-white border-0 d-flex justify-content-between align-items-center">
                                <span class="text-success fw-bold">{{ $price_display }}</span>
                            </div>
                        </div>
                    </div>
                @empty
