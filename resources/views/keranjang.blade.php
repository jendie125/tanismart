...
@section('content')
                        ...
                        <div class="cart-item">
                            {{-- <div class="item-checkbox">
                                <input type="checkbox" class="form-check-input item-select"
                                    data-product-id="{{ $item['idproduk'] }}" data-variant="{{ $item['variant'] }}"
                                    {{ isset($item['selected']) && $item['selected'] ? 'checked' : '' }}>
                            </div> --}}
                            ...
                            <div class="item-details">
                                ...
                                <p class="variant-info">
                                    Variasi: <span class="variant-value">{{ $item['variant'] ?? 'Standard' }}</span>
                                </p>
                            </div>
                            ...
                            <div class="item-quantity">
                                <div class="quantity-control">
                                    <button class="btn btn-sm quantity-btn minus-btn"
                                        onclick="updateQuantity({{ $item['idproduk'] }}, -1, '{{ $item['variant'] }}')">-</button>
                            ...
                            <div class="item-total"
                                id="total-{{ $item['idproduk'] }}-{{ str_replace(' ', '_', $item['variant']) }}">
                                Rp {{ number_format($item['price'] * $item['quantity'], 0, ',', '.') }}
                            </div>
                            <div class="item-actions">
                                <a href="{{ url('keranjang/hapus/' . $item['idproduk'] . '?variant=' . urlencode($item['variant'])) }}"
                                    class="delete-item">
                ...
                <div class="cart-footer">
                    <div class="selection-info">
                        <div class="selection-count">
                            Pilih semua ({{ count(array_filter($cart, fn($item) => $item['selected'] ?? false)) }})
                        </div>
                    </div>
                    <div class="checkout-section">
                        <div class="total-price">
                            Total ({{ count(array_filter($cart, fn($item) => $item['selected'] ?? false)) }} produk):
    ...
    <script>
        ...
        function updateQuantity(productId, change, variant) {
            ...
            if (!isNaN(currentQty)) {
                ...
                if (newQty > 0) {
                    ...
                    xhr.open('POST', '{{ url('keranjang/update') }}', true);
        ...
        function updateCartTotal() {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', '{{ url('keranjang/total') }}', true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
